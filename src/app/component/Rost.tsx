...
export default function Roast({ insult }: Insult) {
  ...
  return (
    ...
    >
        ...
        {formatDistanceToNowStrict(new Date(insult.createdAt), { addSuffix: true })}
      </div>

      <div className="text-[15px] text-[#ffffff] pr-3 mb-3 font-light whitespace-pre-wrap px-2 mt-1">
        {insult.detail}
      </div>
      ...
      <div className="flex gap-8 pb-3 items-center px-2" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center text-xs">
          ...
          <div className={`text-xs ml-1 ${ disliked ? "text-[#74451f]" : "text-[#3b3b3b]"}`}>{insult.dislike}</div>
        </div>
        <div className="flex items-center text-xs">
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="text-xs ml-1 text-[#3b3b3b]">{insult.comments.length}</div>
        </div>
      </div>
